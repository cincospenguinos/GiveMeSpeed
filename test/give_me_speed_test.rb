...
class GiveMeSpeedTest < Minitest::Test
  ...
  def test_message_for_handles_indicates_download_speed
    check = GiveMeSpeed::SpeedCheck.new({ download: 100, upload: 1 })
    assert GiveMeSpeed.tweet_for(check) =~ /I'm paying @comcast for \d+ [a-zA-Z]{3,4} download but I'm only getting \d+ [a-zA-Z]{3,4}. What gives?/
  end

  def test_message_for_handles_indicates_upload_speed
    check = GiveMeSpeed::SpeedCheck.new({ download: 1, upload: 100 })
    assert GiveMeSpeed.tweet_for(check) =~ /I'm paying @comcast for \d+ [a-zA-Z]{3,4} upload but I'm only getting \d+ [a-zA-Z]{3,4}. What gives?/
  end

  def test_message_for_handles_both
    check = GiveMeSpeed::SpeedCheck.new({ download: 100, upload: 100 })
    assert GiveMeSpeed.tweet_for(check) =~ /I'm paying @comcast for \d+ [a-zA-Z]{3,4} download and \d+ [a-zA-Z]{3,4} upload but I'm only getting \d+ [a-zA-Z]{3,4} and \d+ [a-zA-Z]{3,4}. What gives?/
  end
  ...
  def test_message_for_handles_unit_conversion
